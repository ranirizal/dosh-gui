...
if (process.platform === 'win32') {
    ...
    _daemonPath = path.join(__dirname, "bin", process.platform, "doshd.exe");
} else {
    _simpleWalletPath = path.join(__dirname, "bin", process.platform, "simplewallet");
    _daemonPath = path.join(__dirname, "bin", process.platform, "doshd");
}
...
ipcMain.on('wallet:generate', (event, item) => {
    ...
    genWallet.stdout.on('data', (data) => {
        ...
        const walletAddressRegEx = RegExp("Generated new wallet: (X[\\w\\d]{95})");
        ...
        if (RegExp("\\[wallet X[\\d\\w]{2}\\]:").exec(sData) !== null) {
